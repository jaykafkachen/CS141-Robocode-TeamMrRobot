...
public class Name extends AdvancedRobot
{
		...
		{			
			...
			do
			{
				if(target!=null)
				{
					...
					pt = projectPoint(location, moveAngle, Math.min(distance/2, 300));
	...
	public double risk(Point2D.Double point)
	{
		...
		//else
		//{
			riskVal += riskVal*(1+Math.abs(Math.cos(angle(location, riskPoint) - angle(enemyLoc.getLoc(), location))));
		//}
		return riskVal;	
	}
	...
	public void onHitByBullet(HitByBulletEvent e)
	{
		...
		next = projectPoint(location, dir+Math.PI/2, 40);
	}
